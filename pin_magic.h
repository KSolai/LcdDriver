...
// locations, but the ports and bit masks corresponding to each vary among
// boards.  A separate set of pin definitions is given for each supported
// board type.
// When using the TFT break-out board, control pins are configurable but
...
// the shield and break-out configurations -- for the latter, pins were
// chosen to keep the tutorial wiring manageable more than making optimal
// use of ports and bit masks.  So there's a second set of pin definitions
...
// last line is a radio active mu'tant  emotion.
#define DELAY7        \
  asm volatile(       \
    "rjmp .+0" "\n\t" \
    "rjmp .+0" "\n\t" \
...
// When using the TFT break-out board, control pins are configurable.
